...
const calendarSchema = yup.object({
  name: yup.string().required("Calendar name is required").min(3, "Name must be at least 3 characters"),
    ...
    <Box>
      <FormControl fullWidth size="small">
        <Select defaultValue="auto">
          <SelectMenuItem value="auto">Automatically add all invitations to this calendar</SelectMenuItem>
          <SelectMenuItem value="manual">Manually accept invitations</SelectMenuItem>
...
const PermissionsSection = ({ calendar }) => (
  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
    ...
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
      <FormControlLabel
        control={<Switch />}
        label="Make available to public"
      />
    </Box>
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 2, border: '1px solid #e0e0e0', borderRadius: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <Avatar sx={{ width: 32, height: 32, bgcolor: '#6f42c1' }}>
            HP
          </Avatar>
          <Box>
        ...
        <FormControl size="small" sx={{ minWidth: 200 }}>
          <Select defaultValue="manage">
            <SelectMenuItem value="manage">Make changes and manage sharing</SelectMenuItem>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Box>
              ...
              <FormControl fullWidth size="small">
                ...
                >
                  <SelectMenuItem value="busy">See only free/busy (hide details)</SelectMenuItem>
                  <SelectMenuItem value="view">See all event details</SelectMenuItem>
                  <SelectMenuItem value="edit">Make changes to events</SelectMenuItem>
                  <SelectMenuItem value="manage">Make changes and manage sharing</SelectMenuItem>
                </Select>
...
const EventNotificationsSection = ({ calendar }) => {
  const [notifications, setNotifications] = useState([
    { id: 1, type: 'notification', quantity: 30, timeUnit: 'minutes', time: '5:00pm' },
      ...
      {notifications.map((notification) => (
        <Box key={notification.id} sx={{ 
          ...
          <TextField
            ...
            onChange={(e) => updateNotification(notification.id, 'quantity', parseInt(e.target.value) || 1)}
            sx={{ 
              width: 60,
              '& .MuiInputBase-input': { 
                textAlign: 'center',
          ...
          <FormControl size="small" sx={{ minWidth: 80 }}>
            <Select 
              ...
              onChange={(e) => updateNotification(notification.id, 'timeUnit', e.target.value)}
            >
          ...
          <FormControl size="small" sx={{ minWidth: 100 }}>
            ...
            >
              {timeOptions.map((time) => (
                <SelectMenuItem key={time} value={time}>{time}</SelectMenuItem>
              ))}
            </Select>
          </FormControl>
...
const OtherNotificationsSection = ({ calendar }) => {
  const [notifications, setNotifications] = useState([
    { id: 'new', label: 'New events', desc: 'An event is added to this calendar', type: 'none' },
    { id: 'changed', label: 'Changed events', desc: 'An event on this calendar is changed', type: 'none' },
    { id: 'canceled', label: 'Canceled events', desc: 'An event on this calendar is cancelled', type: 'none' },
    { id: 'responses', label: 'Event responses', desc: 'Guests respond to an event on this calendar', type: 'none' },
    { id: 'agenda', label: 'Daily agenda', desc: 'Receive a daily email with the agenda for this calendar', type: 'none' },
  ]);
...
const CalendarSidebar = ({ 
  ...
  const [selectedCalendarForDetails, setSelectedCalendarForDetails] = useState(null);
  ...
  useEffect(() => {
    if (selectedCalendarForDetails) {
      reset({
        ...
        type: selectedCalendarForDetails.securityVisibility || selectedCalendarForDetails.type || "private",
    ...
    >
          ...
          {['Private', 'Shared', 'Public'].map((type) => (
            <Button
              ...
              sx={{
                ...
                fontSize: '0.75rem',
                py: 0.5,
                backgroundColor: calendarType === type ? '#6f42c1' : 'transparent',
                color: calendarType === type ? '#fff' : '#6c757d',
                borderColor: calendarType === type ? '#6f42c1' : '#dee2e6',
                '&:hover': {
                  backgroundColor: calendarType === type ? '#5a32a3' : '#f8f9fa',
                },
              }}
            >
              {type}
        ...
        <List sx={{ p: 0 }}>
          ...
          {createdCalendars
            .filter((calendar) => {
              ...
              const calendarTypeValue = calendar.securityVisibility || calendar.type;
              return typeMapping[calendarTypeValue] === calendarType;
            })
            .length > 0 && (
            <Box sx={{ mb: 3 }}>
              {/* Section Header */}
              ...
              <Box
                sx={{
                  ...
                  '&::before': {
                    ...
                    right: 0,
                    height: 2,
                    background: 'linear-gradient(90deg, #6f42c1, #4285f4, #6f42c1)',
                  ...
                  '&::after': {
                    ...
                    right: 0,
                    bottom: 0,
                    background: 'radial-gradient(circle at 20% 80%, rgba(111, 66, 193, 0.03) 0%, transparent 50%)',
                    borderRadius: 2.5,
                    pointerEvents: 'none',
                  },
                }}
                ...
                {createdCalendars
                  ...
                  >
                    <Card
                      sx={{
                        width: '100%',
                        background: selectedCalendar === calendar.name 
                          ? 'linear-gradient(135deg, #f0f4ff 0%, #e8f0ff 100%)'
                          : 'linear-gradient(135deg, #ffffff 0%, #fafbfc 100%)',
                        border: selectedCalendar === calendar.name 
                          ? '2px solid #6f42c1' 
                          : '1px solid #e9ecef',
                        borderRadius: 2,
                        boxShadow: selectedCalendar === calendar.name
                        ...
                        '&::before': {
                          ...
                          right: 0,
                          height: 2,
                          background: `linear-gradient(90deg, ${calendar.colour || calendar.color || '#4285f4'}, ${calendar.colour || calendar.color || '#4285f4'}80)`,
                          opacity: selectedCalendar === calendar.name ? 1 : 0.7,
                      ...
                      onClick={() => onCalendarSelect && onCalendarSelect(calendar.name)}
                    >
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Box
                              sx={{
                                ...
                                height: 12,
                                borderRadius: '50%',
                                background: `linear-gradient(135deg, ${calendar.colour || calendar.color || '#4285f4'}, ${calendar.colour || calendar.color || '#4285f4'}cc)`,
                                mr: 1,
      ...
      >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
              <Box
                sx={{
                  ...
                  height: 12,
                  borderRadius: '50%',
                  backgroundColor: selectedCalendarForMenu.colour || selectedCalendarForMenu.color || '#4285f4',
                }}
              />
              <Typography variant="body2" sx={{ fontWeight: 600, color: '#333' }}>
                {selectedCalendarForMenu.name}
              </Typography>
              <VisibilityIcon sx={{ fontSize: 16, color: '#6c757d', ml: 'auto' }} />
            </Box>
            <Typography variant="caption" sx={{ color: '#6c757d' }}>
              {selectedCalendarForMenu.type === 'default' ? 'Default Calendar' : 'Created Calendar'}
            </Typography>
          </Box>
        )}
        ...
        {menuItems.map((item, index) => [
          ...
          (index === 3 || index === 6) && <Divider key={`divider-${item.id}`} />
        ].filter(Boolean))}
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="h6" sx={{ fontWeight: 600, color: 'white' }}>
              {selectedCalendarForDetails ? 'Edit Calendar' : 'Create New Calendar'}
            </Typography>
          </Box>
        </DialogTitle>
                ...
                {[
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      py: 1,
                      borderRadius: 1.5,
                      border: activeSection === item.id ? '1px solid #6f42c1' : '1px solid #e0e0e0',
                      background: activeSection === item.id ? '#6f42c1' : '#fff',
                      ...
                      '&:hover': {
                        background: activeSection === item.id ? '#5a32a3' : '#f8f9fa',
                ...
                {activeSection === 'notifications' && (
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    ...
                    <Box sx={{ 
                      ...
                      <EventNotificationsSection calendar={selectedCalendarForDetails} />
                    </Box>
                    ...
                    <Box sx={{ 
                      ...
                      <AllDayNotificationsSection calendar={selectedCalendarForDetails} />
                    </Box>
                    ...
                    <Box sx={{ 
                      ...
                      <OtherNotificationsSection calendar={selectedCalendarForDetails} />
                    </Box>
                  </Box>
                )}
              </Box>

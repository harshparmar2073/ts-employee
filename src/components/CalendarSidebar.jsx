...
const [showConnectedCalendars, setShowConnectedCalendars] = useState(true);

const [disconnectLoading, setDisconnectLoading] = useState(null); // holds the mapped-row id when disconnecting
  ...
  // Keep the edit dialog's selected calendar details in sync when parent list updates
  useEffect(() => {
    if (
      calendarFormOpen &&
      selectedCalendarForDetails &&
      selectedCalendarForDetails.id
    ) {
      const latest = createdCalendars.find(
        (c) => c.id === selectedCalendarForDetails.id
      );
      if (latest) setSelectedCalendarForDetails(latest);
    }
  }, [createdCalendars, calendarFormOpen]);

  
const getConnectedExternalCalendars = () => {
  // Use the selected calendar for details, or fall back to the currently selected calendar
  const sourceCal = selectedCalendarForDetails || 
    (selectedCalendar?.id 
      ? createdCalendars.find((c) => String(c.id) === String(selectedCalendar.id))
      : null) ||
    createdCalendars?.[0] ||
    null;

  if (!sourceCal) {
    console.log("No source calendar found for external connections");
    return [];
  }

  console.log("Source calendar for external connections:", sourceCal);

  const rows = [];

  // Check if calendar has external calendar connections
  // Handle array format (multiple connections)
  if (Array.isArray(sourceCal.externalCalendar) && sourceCal.externalCalendar.length > 0) {
    console.log("Found external calendar array:", sourceCal.externalCalendar);
    
    sourceCal.externalCalendar.forEach((external, index) => {
      const type = (external.externalCalendarType || external.type || "").toUpperCase();
      if (type) {
        rows.push({
          id: `${type}_${index}`,
          name: type === "GOOGLE" ? "Google Calendar" : 
                type === "MICROSOFT" ? "Microsoft Calendar" : 
                `${type} Calendar`,
          type: type,
          email: external.externalCalendarEmail || 
                 external.externalEmail || 
                 sourceCal.externalCalendarEmail || 
                 "Email not available",
          createdAt: external.externalLinkCreatedAt || 
                    external.createdAt || 
                    external.connectedAt ||
                    sourceCal.externalLinkCreatedAt ||
                    sourceCal.createdDate ||
                    new Date().toISOString(),
          isActive: external.isActive !== false, // Default to true if not specified
        });
      }
    });
  }

  // Handle single external calendar (flattened structure)
  if (sourceCal.externalCalendarType && !rows.some(r => r.type === sourceCal.externalCalendarType.toUpperCase())) {
    const type = sourceCal.externalCalendarType.toUpperCase();
    rows.push({
      id: `${type}_single`,
      name: type === "GOOGLE" ? "Google Calendar" : 
            type === "MICROSOFT" ? "Microsoft Calendar" : 
            `${type} Calendar`,
      type: type,
      email: sourceCal.externalCalendarEmail || "Email not available",
      createdAt: sourceCal.externalLinkCreatedAt ||
                sourceCal.createdDate ||
                sourceCal.updatedAt ||
                new Date().toISOString(),
      isActive: sourceCal.externalCalendarActive !== false,
    });
  }

  // Check for Google-specific fields
  if (sourceCal.googleCalendarId || sourceCal.isGoogleConnected) {
    if (!rows.some(r => r.type === "GOOGLE")) {
      rows.push({
        id: "GOOGLE_legacy",
        name: "Google Calendar",
        type: "GOOGLE",
        email: sourceCal.googleCalendarEmail || 
               sourceCal.externalCalendarEmail || 
               "Email not available",
        createdAt: sourceCal.googleConnectedAt ||
                  sourceCal.externalLinkCreatedAt ||
                  sourceCal.createdDate ||
                  new Date().toISOString(),
        isActive: sourceCal.isGoogleConnected !== false,
      });
    }
  }

  // Check for Microsoft-specific fields
  if (sourceCal.microsoftCalendarId || sourceCal.isMicrosoftConnected) {
    if (!rows.some(r => r.type === "MICROSOFT")) {
      rows.push({
        id: "MICROSOFT_legacy", 
        name: "Microsoft Calendar",
        type: "MICROSOFT",
        email: sourceCal.microsoftCalendarEmail ||
               sourceCal.externalCalendarEmail ||
               "Email not available",
        createdAt: sourceCal.microsoftConnectedAt ||
                  sourceCal.externalLinkCreatedAt ||
                  sourceCal.createdDate ||
                  new Date().toISOString(),
        isActive: sourceCal.isMicrosoftConnected !== false,
      });
    }
  }

  // Sort by creation date (most recent first) but keep all entries
  rows.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

  console.log("Final external calendar rows:", rows);
  return rows;
};

  
  const handleGoogleConnectedLocal = useCallback((payload) => {
  // keep UI simple: show connected panel and bubble event up
  setShowConnectedCalendars(true);
  if (onGoogleCalendarConnected) onGoogleCalendarConnected(payload);
}, [onGoogleCalendarConnected]);

const handleMicrosoftConnectedLocal = useCallback((payload) => {
  setShowConnectedCalendars(true);
  if (onMicrosoftCalendarConnected) onMicrosoftCalendarConnected(payload);
}, [onMicrosoftCalendarConnected]);

const handleDisconnectFromRow = useCallback(async (mappedRow) => {
  const calendarId = selectedCalendarForDetails?.id || selectedCalendar?.id || (createdCalendars?.[0]?.id ?? null);
  if (!calendarId) {
    console.warn("No calendarId available for disconnect");
    return;
  }
  try {
    setDisconnectLoading(mappedRow.id);
    await axiosService.post("/calendar/google-disconnect", {  email: mappedRow.email ,calendarId});

    // Optimistically mark as disconnected but keep the row visible
    setSelectedCalendarForDetails(prev => {
      if (!prev) return prev;
      const updated = { ...prev };

      if (Array.isArray(updated.externalCalendar) && updated.externalCalendar.length > 0) {
        updated.externalCalendar = updated.externalCalendar.map(ex => {
          const t = (ex.externalCalendarType || ex.type || "").toUpperCase();
          if (t === mappedRow.type) {
        ...
        // If there was no array entry, add a synthetic disconnected entry so UI reflects status
        updated.externalCalendar = [
          ...(updated.externalCalendar || []),
          {
            externalCalendarType: mappedRow.type,
            externalCalendarEmail: mappedRow.email || "",
            externalLinkCreatedAt: mappedRow.createdAt || new Date().toISOString(),
            isActive: false,
          },
        ];
      }

      if (mappedRow.type === "GOOGLE") {
        updated.isGoogleConnected = false;
        // Do not wipe IDs/emails so we can still show metadata if needed
      } else if (mappedRow.type === "MICROSOFT") {
        updated.isMicrosoftConnected = false;
      }

      return updated;
    });

    // Notify parent so it can refresh canonical data
    if (mappedRow.type === "GOOGLE" && typeof onGoogleCalendarDisconnected === "function") {
      onGoogleCalendarDisconnected({ calendarId });
    }
    if (mappedRow.type === "MICROSOFT" && typeof onMicrosoftCalendarDisconnected === "function") {
      onMicrosoftCalendarDisconnected({ calendarId });
    }
  } catch (err) {
...
}, [selectedCalendarForDetails, selectedCalendar, createdCalendars, onGoogleCalendarDisconnected, onMicrosoftCalendarDisconnected]);
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Typography variant="h6" sx={{ fontWeight: 600, color: "white" }}>
              {selectedCalendarForDetails
                ? `Edit ${selectedCalendarForDetails.name } Calendar`
                : "Create New Calendar"}
            </Typography>
          </Box>
        </DialogTitle>
    ...
    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      {showConnectedCalendars && (
        <Button
          variant="contained"
          size="small"
          onClick={() => setShowConnectedCalendars(false)}
          sx={{
            color: "#fff",
            borderRadius: 2,
            textTransform: "none",
        ...
        >
          Connect New External Calendar
        </Button>
      )}
    </Box>

    {showConnectedCalendars ? (
      // Connected Calendars Data Table
      <Box>
        <TableContainer 
          component={Paper} 
          sx={{ 
 ...
 <TableRow
    sx={{
      backgroundColor: "#f1f5ff",         // subtle bluish tint
      borderBottom: "1px solid #e6ecff",  // soft divider
  ...
  >                <TableCell sx={{ fontWeight: 600, color: "#495057", py: 2 }}>
          ...
          <TableBody>
  {getConnectedExternalCalendars().length > 0 ? (
    getConnectedExternalCalendars().map((calendar) => (
      <TableRow 
        key={calendar.id} 
        sx={{ 
          "&:hover": { backgroundColor: "#f8f9fa" },
          opacity: calendar.isActive === false ? 0.6 : 1 
        }}
      >
<TableCell sx={{ py: 2 }}>
            ...
            <Box
              sx={{
                ...
                height: 32,
                borderRadius: "8px",
                background: calendar.type === "GOOGLE" 
                  ? "linear-gradient(135deg, #4285f4 0%, #34a853 100%)"
                  : calendar.type === "MICROSOFT"
                  ? "linear-gradient(135deg, #0078d4 0%, #00a1f1 100%)"
                  : "linear-gradient(135deg, #6c757d 0%, #495057 100%)", // fallback
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                color: "white",
            ...
            >
              {calendar.type === "GOOGLE" ? "G" : 
               calendar.type === "MICROSOFT" ? "M" : 
               calendar.type?.charAt(0) || "?"}
            </Box>
            <Box>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Chip
                  label={
                    calendar.isActive === false ? "Disconnected" :
                    calendar.type === "GOOGLE" ? "Connected to Google" : 
                    calendar.type === "MICROSOFT" ? "Connected to Microsoft" :
                    "Connected"
                  }
                  size="small"
                  sx={{
                    ...
                    fontSize: "0.72rem",
                    fontWeight: 600,
                    color: calendar.isActive === false ? "#f44336" :
                           calendar.type === "GOOGLE" ? "#2e7d32" : 
                           calendar.type === "MICROSOFT" ? "#2140e8" :
                           "#6c757d",
                    bgcolor: calendar.isActive === false ? "#ffebee" :
                             calendar.type === "GOOGLE" ? "#e8f5e8" : 
                             calendar.type === "MICROSOFT" ? "#e8f0fe" :
                             "#f5f5f5",
                    border: calendar.isActive === false ? "1px solid #f44336" :
                            calendar.type === "GOOGLE" ? "1px solid #4caf50" : 
                            calendar.type === "MICROSOFT" ? "1px solid #4f6bed" :
                            "1px solid #6c757d",
                    borderRadius: 1.5,
                  }}
                />
              ...
              <Typography variant="caption" sx={{ color: "#6c757d" }}>
                Connected on {calendar.createdAt ? 
...
<TableCell sx={{ py: 2 }}>
  <Button
    variant="outlined"
    size="small"
    onClick={() => handleDisconnectFromRow(calendar)}
    disabled={disconnectLoading === calendar.id}
    sx={{
      borderColor: "#dc3545",
      color: "#dc3545",
      borderRadius: 2,
  ...
  >
    {disconnectLoading === calendar.id ? "Disconnecting..." : "Disconnect"}
  </Button>
</TableCell>
              </TableRow>
    ))
      ...
      <TableCell colSpan={2} sx={{ py: 4 }}>
        ...
        >
          ...
          <Typography variant="body2" sx={{ color: "#6c757d", mb: 2 }}>
            Connect your Google or Microsoft calendar to keep events in sync.
          </Typography>
          <Button
            variant="contained"
            size="small"
      ...
      <Box>
        ...
        >
          ...
          <Box sx={{ display: "flex", justifyContent: "flex-start" }}>
            ...
            onSuccess={handleGoogleConnectedLocal}
            />
          </Box>
        </Box>
        ...
        >
          ...
          <Box sx={{ display: "flex", justifyContent: "flex-start" }}>
            ...
            calendarId={selectedCalendar?.id || (createdCalendars?.length > 0 ? createdCalendars[0]?.id : null)}
            calendarData={selectedCalendar || (createdCalendars?.length > 0 ? createdCalendars[0] : null)}
            onSuccess={handleMicrosoftConnectedLocal}
            />
          </Box>
        </Box>
      </Box>
